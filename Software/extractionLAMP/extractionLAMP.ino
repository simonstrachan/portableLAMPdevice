...
//LAMP values
double targetTemp = 0;
const double lampTemp = 78.5;
const double realLamp = 65;
const double extractionTemp = 110;
const double realExtraction = 95;
float endTime = 30;
const float lampTime =  ;
const float extractionRampTime = 30;
const float extractionTime = 5;
...
void loop(void) {
  ...
  //Press 's' to start experiment, 'r' to start extraction, 'e' to start extraction ramp and 'l' to start LAMP
  if (!running && inChar == 's') {
    running = true;
    startTime = millis();
    prev_timeSeconds = 0;
  } else if (!ready && inChar == 'e') {
    ready = true;
    EorL = 1;
    targetTemp = extractionTemp;
    endTime = extractionRampTime;
    startTime = millis();
    prev_timeSeconds = 0;
  } else if (inChar == 'l') {
    ready = true;
...
void printTimer() {
  ...
  if (timer > prev_timeSeconds) {
    ...
    Serial.print("s, ");
    if(EorL == 1){
      Serial.print(steinhart - (targetTemp - realExtraction));
    } else if(EorL == 2){
      Serial.print(steinhart - (targetTemp - realLamp));
    } else {
...
void RGBsensor(uint8_t sensorNum) {
  ...
  tube[sensorNum].sum -= tube[sensorNum].readings[tube[sensorNum].index];  //removes oldest entry
  tube[sensorNum].readings[tube[sensorNum].index] = zeroCIV;               //Add newest to window
  tube[sensorNum].sum += zeroCIV;                                          //Add newest to sum
  tube[sensorNum].index = (tube[sensorNum].index + 1) % window_size;       //Increment the index or wrap to 0
  tube[sensorNum].averaged = tube[sensorNum].sum / window_size;


  //Check for increase in CIV
  if (tube[sensorNum].readingCheck == 2) {
    uint8_t previousIndex = tube[sensorNum].currentIndex % numReadings;
    tube[sensorNum].currentReadings = tube[sensorNum].averaged;

    if (timeMinutes >= 5 && (tube[sensorNum].currentReadings - (tube[sensorNum].pos_readings[previousIndex] + 0.35)) / numReadings > 0) {
      tube[sensorNum].pos_count++;
    } else {
      tube[sensorNum].pos_count = 0;
    }
    tube[sensorNum].pos_readings[tube[sensorNum].currentIndex] = tube[sensorNum].currentReadings;
    tube[sensorNum].currentIndex = (tube[sensorNum].currentIndex + 1) % numReadings;
    tube[sensorNum].readingCheck = 0;
  } else {
    tube[sensorNum].readingCheck++;
  }
}
